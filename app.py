...
# Add FFmpeg to the system path automatically
add_paths()
...
def sender():
    ...
    initialize_folders() # Ensure folders exist in this execution
    ...
    # Using 'ffmpeg' directly works because of add_paths()
    cmd = [
        "ffmpeg", "-i", video_path, 
        "-vf", "drawtext=text='SHIELD-ID':x=w-tw-20:y=h-th-20:fontcolor=white@0.02",
        "-f", "segment", "-segment_time", "30", # Smaller segments for Vercel
        output_template
    ]
    ...
    with ThreadPoolExecutor(max_workers=2) as exe: # Lower workers for serverless
...
def run_receiver_task():
    ...
    if not vault_files: return jsonify({"status": "Empty", "message": "Nothing in vault."})
    ...
    if current_segments:
        ...
        with open(concat_file, "w") as f:
            for ts in current_segments: f.write(f"file '{ts}'\n")
